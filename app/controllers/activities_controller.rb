class ActivitiesController < ApplicationController
  ...
  before_filter only: [:index] do 
    @recent_activities = Activity.by_most_recent.active.limit(5)
    @popular_activities = Activity.by_popularity.active.limit(5)
    @popular_neighborhoods = Neighborhood.by_popularity.limit(5)
  end

  def search
    @neighborhood = Neighborhood.find_by_id(params[:by_neighborhood])
    @activities = Activity.active.find_all_by_neighborhood_id(params[:by_neighborhood])
    @nearby_activities = Activity.by_distance(@neighborhood.latitude, @neighborhood.longitude).active - @activities
  end
