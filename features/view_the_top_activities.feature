Feature: View the top activities
  ...
  Scenario: when there are activities with different number of adventurers
    Given there is an event called "Aula de forró na praça São Salvador" with 2 attendees
    And there is an event called "Velejada na Baía de Guanabara" with 4 attendees
    And there is an event called "Feijoada da Portela" with 3 attendees
